...
def validate_least_flights_earliest(planner, start_city, end_city, t1, t2):
    ...
    if route == []:
        print("Validation failed: Planner returned no route while at least one route exists.")
        print(f'start:{start_city}, end:{end_city}')
        print('one possible route:', [f.flight_no for f in all_routes[0]])
        return False
...
def run_tests():
    ...
    if not check_problem_1(planner, t1, t2, num_cities):
        return
    if not check_problem_2(planner, t1, t2, num_cities):
        return
    if not check_problem_3(planner, t1, t2, num_cities):
        return
